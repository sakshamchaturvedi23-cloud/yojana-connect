...
export function AuthCard() {
  ...
  const from =
    rawFrom && rawFrom.startsWith("/") && rawFrom !== "/login"
      ? rawFrom
      : "/";
  const { isAuthenticated, isLoading, loginAsDemo } = useAuth();

  // If already authenticated and visiting /login with an explicit deep-link destination other than "/", redirect cleanly
  useEffect(() => {
    if (!isLoading && isAuthenticated && rawFrom && rawFrom !== "/" && rawFrom !== "/login") {
      router.replace(from);
    }
  }, [isLoading, isAuthenticated, rawFrom, router, from]);
  ...
  // Catch errors passed from OAuth callback or API redirect
  const errorFromQuery = searchParams.get("error");
  useEffect(() => {
    if (errorFromQuery) {
      if (errorFromQuery === "auth_callback_failed") {
        setErrorMessage("Authentication failed during provider verification. Please try again.");
      } else if (errorFromQuery === "invalid_provider") {
        setErrorMessage("Invalid authentication provider requested.");
      } else {
  ...
  // Trigger Backend OAuth Authentication Endpoints
  ...
  const handleDemoSignIn = async () => {
    ...
    try {
      await loginAsDemo();
      setSuccessMessage("Signed in as Demo Citizen! Redirecting...");
      setTimeout(() => {
        router.push(from);
        router.refresh();
      }, 500);
  ...
  const handleMagicLink = async (email: string) => {
    ...
    try {
      const res = await magicLinkApi(email.trim(), from);
      if (res.success) {
        setSuccessMessage(res.message || "Magic link sent to your email! Please check your inbox.");
      } else {
      ...
      <div className="relative rounded-3xl border border-neutral-800 bg-neutral-900/80 p-6 sm:p-8 backdrop-blur-xl shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={isDemoSubmitting || isOAuthGoogle || isOAuthGithub || isMagicSubmitting}
            className="w-full rounded-2xl border border-neutral-800 bg-neutral-950/60 px-4 py-2.5 text-xs sm:text-sm font-medium text-neutral-300 hover:text-white hover:bg-neutral-800/80 hover:border-neutral-700 transition flex items-center justify-center gap-2 backdrop-blur-md active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isDemoSubmitting ? (
              ...
              <>
                ...
                <span>Quick Demo Citizen Access</span>
              </>
            )}
          </button>
        </div>
