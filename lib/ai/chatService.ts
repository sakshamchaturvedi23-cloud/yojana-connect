...
import { checkGreetingOrSmallTalk } from "./greetingHelper";
...
export async function answerChat(options: AnswerChatOptions): Promise<ChatAnswerResult> {
  ...
  // 0. Greeting & Small Talk Guardrail: Bypass RAG and return polite conversational response
  const smallTalk = checkGreetingOrSmallTalk(message, answerLanguage);
  if (smallTalk) {
    return {
      answer: smallTalk.answer,
      language: answerLanguage,
      sources: [],
      schemes: schemeId ? [schemeId] : [],
      fallback: false,
    };
  }
